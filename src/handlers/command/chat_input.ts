...
export async function chatInputApplicationCommandHandler<E extends Env>(
  ...
  if (!('shouldDefer' in command.runner) || !command.runner.shouldDefer) {
    const callback =
      'callback' in command.runner ? command.runner.callback : command.runner;
    return await callback(interaction, inputMap);
  }

  const promise = async (
    callback: ChatInputCommandCallback<E, ChatInputCommandArguments<E>, void>,
  ) => {
  ...
  interaction.ctx.hono.executionCtx.waitUntil(promise(command.runner.callback));
